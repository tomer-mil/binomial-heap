...
class BinomialHeapTheoryTest {
    ...
    public ArrayList<Integer> generateTheoryKeys(int i) {
        ...
        for (int j = 1; j <= totalNumOfKeys; j++) {
            keys.add(j);
        }
        return keys;
    }
...
//        heap.print();
    ...
    void testTheoryInsert() {
        ...
        // int stop = (int) Math.pow(2, 5) - 1;
        Collections.shuffle(theoryKeys);
        // for (int j=n; j>=1; j--) {
        //     this.theoryTestingHeap.insert(j, "");
        // }
        for (int key :theoryKeys){
            this.theoryTestingHeap.insert(key, "");
        }
        for (int j=1; j<=(n/2); j++){
            sumRanks += this.theoryTestingHeap.min.rank;
            this.theoryTestingHeap.deleteMin();
        }
        // while (this.theoryTestingHeap.size > stop) {
        //     sumRanks += this.theoryTestingHeap.min.rank;
        //     this.theoryTestingHeap.deleteMin();;
        //}
